```
📁 Annotated Directory Structure

├─ admin/                                  # Admin module
│  ├─ admin_dashboard.php            # Admin dashboard
│  ├─ activity_log.php               # Tracks admin actions and activity history
│  ├─ admin_format.css               # Admin header and sidebar styles
│  ├─ admin_format.php               # Admin header and sidebar layout
│  ├─ alumni_management.php          # Batch cards display with search/filter features
│  ├─ batch_alumni.php               # Alumni list for a given batch and management actions
│  ├─ check_status.php               # Submission check
│  ├─ get_alumni_details.php         # Hover preview of alumni details
│  ├─ get_documents.php              # For admin document viewing
│  ├─ monitor_submission_status.php  # Monitor sub stat
│  ├─ report_generation.php          # Report generation
│  ├─ set_schedule.php               # as the name suggests...
│  ├─ submission_review.php          # Hover preview of alumni details
│  ├─ submission_schedule.php        # Submission sched
│  └─ update_status.php              # Backend logic; handles the approval/rejection and notif
│
├─ alumni/                                # Alumni module
│  ├─ alumni_dashboard.php           # Alumni dashboard
│  ├─ alumni_employment.php          # Emp info page
│  ├─ alumni_format.css              # Alumni header and sidebar styles
│  ├─ alumni_format.php              # Alumni header and sidebar layout
│  ├─ alumni_profile.php             # Profile management page
│  ├─ mark_all_notifications_read.php # Mark all as read
│  ├─ alumni_employment.php          # Backend logic for employment info page
│  └─ update_profile.php             # Backend logic for Profile management page
```
____________________

here nga branch rami mag commit, for a vv trivial reason...
para d matabunan amung recent PR title hahalol
<img width="728" height="259" alt="image" src="https://github.com/user-attachments/assets/833b5689-df5d-44ba-abd8-d838f5f9bb4a" />