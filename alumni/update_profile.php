...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // DEBUGGING PURPOSES
  /*  echo "<pre>";
    var_dump($_POST);  // Check all POST data, especially 'barangay_id'
    echo "</pre>"; */
...
if ($can_update) {
    ...
    error_log("Raw barangay_id: '$raw_barangay_id' (hex: " . bin2hex($raw_barangay_id) . ") for user_id $user_id");

    if ($barangay_id && $street_details && $zip_code) {
        // Sanitize barangay_id
        $barangay_id = preg_replace('/[^a-zA-Z0-9]/', '', $barangay_id);
        if (strlen($barangay_id) !== 9) {
            error_log("Invalid barangay_id length: '$barangay_id' (hex: " . bin2hex($barangay_id) . ") for user_id $user_id");
            header("Location: alumni_profile.php?error=" . urlencode("Invalid Barangay ID length: must be 9 characters"));
            exit;
        }
        error_log("Sanitized barangay_id: '$barangay_id' (hex: " . bin2hex($barangay_id) . ") for user_id $user_id");
        ...
        try {
            ...
            $check_stmt = $conn->prepare("SELECT barangay_id FROM table_barangay WHERE BINARY barangay_id = ?");
            $check_stmt->bind_param("s", $barangay_id);
            $check_stmt->execute();
            $check_result = $check_stmt->get_result();
            if ($check_result->num_rows === 0) {
                throw new Exception("Invalid barangay_id: '$barangay_id' (hex: " . bin2hex($barangay_id) . ")");
            }
            $check_stmt->close();

            // Proceed with insert or update
            if ($address_id) {
                $stmt = $conn->prepare("UPDATE address SET barangay_id = ?, street_details = ?, zip_code = ? WHERE address_id = ?");
                if (!$stmt->bind_param("issi", $barangay_id, $street_details, $zip_code, $address_id)) {
                    throw new Exception("Bind param failed for UPDATE: " . $stmt->error);
                }
            } else {
                $stmt = $conn->prepare("INSERT INTO address (barangay_id, street_details, zip_code) VALUES (?, ?, ?)");
                if (!$stmt->bind_param("iss", $barangay_id, $street_details, $zip_code)) {
                    throw new Exception("Bind param failed for INSERT: " . $stmt->error);
                }
            }
            if (!$stmt->execute()) {
            ...
            $conn->rollback();
            error_log("Address error for user_id $user_id, barangay_id '$barangay_id' (hex: " . bin2hex($barangay_id) . "): " . $e->getMessage());
            header("Location: alumni_profile.php?error=" . urlencode("Address update failed: " . $e->getMessage()));
            exit;
        }
    } elseif ($barangay_id || $street_details || $zip_code) {
        error_log("Incomplete address fields for user_id $user_id: barangay_id='$barangay_id', street_details='$street_details', zip_code='$zip_code'");
        header("Location: alumni_profile.php?error=" . urlencode("All address fields are required if any is provided."));
        exit;
    }
}
    ...
    if ($can_update) {
        ...
        if (($employment_status === 'Employed' || $employment_status === 'Employed & Student') && $job_title) {
            ...
            $stmt = $conn->prepare("REPLACE INTO employment_info (user_id, job_title_id, company_name, salary_range, business_type) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("iisss", $user_id, $job_title_id, $company, $salary, $business_type);
            $stmt->execute();
        } elseif ($employment_status === 'Self-Employed' && ($business_type || $salary)) {
            $stmt = $conn->prepare("REPLACE INTO employment_info (user_id, job_title_id, company_name, salary_range, business_type) VALUES (?, NULL, NULL, ?, ?)");
    ...
    if ($can_update) {
        ...
        if ($can_update || in_array('COR', $rejected_docs)) {
            ...
            if ($can_update) {  // Only for full updates, not re-uploads
