...
// Fetch employment_status (only if profile exists)
$profile = ['employment_status' => 'Not Set'];
if (!empty($profile_info)) {
    $profile = ['employment_status' => $profile_info['employment_status'] ?? 'Not Set'];
}

// Fetch latest document status (only if profile exists)
...
<!-- Regional Employment Overview (BSIT Alumni) -->
<!-- <div class="bg-white shadow rounded-lg p-6 mb-6">
    ...
    <?php if (empty($regions_data)): ?>
        <p class="text-gray-500">No alumni data available yet. Update your profile to contribute!</p>
    <?php else: ?>
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Alumni</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <?php foreach ($regions_data as $region): ?>
                        <tr class="<?php echo ($region['employment_rate'] < 70) ? 'bg-red-50' : ''; ?>">
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900"><?php echo htmlspecialchars($region['region_name']); ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?php echo $region['alumni_count']; ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><?php echo $region['employed_count']; ?></td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm <?php echo ($region['employment_rate'] < 70) ? 'text-red-600' : 'text-green-600'; ?>">
                                <?php echo $region['employment_rate']; ?>%
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
        ...
        <p class="text-xs text-gray-500 mt-2">Data as of October 14, 2025. Rates based on verified employment info.</p>
    <?php endif; ?>
</div> -->
...
<div class="space-y-6">
    ...
    <?php else: ?>
        <?php if (!empty($profile_info)): ?>
            <!-- Stats Grid (shown only if profile exists and no welcome is triggered) -->
